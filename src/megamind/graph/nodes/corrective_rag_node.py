...
def _detect_error(tool_message: ToolMessage) -> tuple[bool, str | None]:
    ...
    Args:
        tool_message: The ToolMessage to analyze for error patterns

    Returns:
        A tuple of (has_error, error_description) where:
        - has_error (bool): True if an error pattern was detected, False otherwise
        - error_description (str | None): A snippet of text around the error if detected,
...
def _extract_doctype_from_context(messages: list) -> str | None:
    ...
    Args:
        messages: List of conversation messages (AIMessage, HumanMessage, ToolMessage, etc.)
                  to search for DocType information

    Returns:
        The extracted DocType name in Title Case (e.g., "Sales Order") if found,
        None if no DocType could be extracted from the messages
    """
    ...
    for msg in reversed(messages):
        if isinstance(msg, AIMessage) and hasattr(msg, 'tool_calls') and msg.tool_calls:
            for tool_call in msg.tool_calls:
                ...
                # Pattern: operation_(doctype_name) - case insensitive
                doctype_match = re.search(r"(?i)(?:create_|update_|delete_|get_)([a-zA-Z_]+)", tool_name)
                if doctype_match:
...
async def _generate_corrective_query(
    ...
    Args:
        error_description: The error message or snippet extracted from the tool result
        tool_name: The name of the tool that failed (e.g., "create_sales_order")
        doctype: Optional DocType name being operated on (e.g., "Sales Order")
        original_messages: List of recent conversation messages for context

    Returns:
        A targeted knowledge search query string optimized to retrieve information
        that will help fix the error. Falls back to a generic query if LLM generation fails.
    """
...
async def _retrieve_corrective_knowledge(
    ...
    Args:
        query: The corrective knowledge search query generated by LLM
        doctype: Optional DocType name to filter results (e.g., "Sales Order")

    Returns:
        Formatted string containing corrective knowledge entries with titles,
        summaries, and content. Returns empty string if no results found or
        if retrieval fails.
    """
...
async def corrective_rag_node(state: AgentState, config: RunnableConfig) -> Dict[str, Any]:
    """
    ...
    # Find the last tool message and its index
    last_tool_message = None
    tool_name = None
    last_tool_message_idx = None

    for idx in range(len(messages) - 1, -1, -1):
        msg = messages[idx]
        if isinstance(msg, ToolMessage):
            last_tool_message = msg
            last_tool_message_idx = idx
            break

    if last_tool_message is not None and last_tool_message_idx is not None:
        # Search backwards from the tool message for the corresponding AIMessage
        for ai_idx in range(last_tool_message_idx - 1, -1, -1):
            ai_msg = messages[ai_idx]
            if isinstance(ai_msg, AIMessage) and hasattr(ai_msg, 'tool_calls') and ai_msg.tool_calls:
                for tool_call in ai_msg.tool_calls:
                    if tool_call.get("id") == last_tool_message.tool_call_id:
                        tool_name = tool_call.get("name")
                        break
            if tool_name:
                break
