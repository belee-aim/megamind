...
async def _extract_stock_request(messages: List[Any], model_name: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            result = internal_llm.invoke(
                messages_for_llm,
                config={"configurable": {"streaming": False}}
            )
        except:
...
async def _validate_and_search_item(item_code: str, mcp_tools: List[Any]) -> Dict[str, Any]:
    """
...
def _format_success_message(result: Dict[str, Any], item_info: Dict[str, Any], quantity: float) -> str:
    """
...
async def smart_stock_movement_node(state: StockMovementState, config: RunnableConfig) -> Dict[str, Any]:
    """
    ...
    if not company:
        ...
        try:
            ...
            if not company:
                return {
                    "messages": [AIMessage(content="Компанийн мэдээлэл олдсонгүй. Системийн админтай холбогдоно уу.")]
                }
        except Exception as e:
            return {
                "messages": [AIMessage(content=f"Компанийн мэдээлэл авахад алдаа гарлаа: {e}")]
            }
    ...
    try:
        ...
        extracted_data = await _extract_stock_request(messages, configurable.query_generator_model)
        logger.info(f"Extracted data: {extracted_data}")
        
        # Step 2: Validate and search item
        logger.info(f"Validating item: {extracted_data['item_code']}")
        item_info = await _validate_and_search_item(extracted_data["item_code"], mcp_tools)
        
        if not item_info["found"]:
            logger.warning(f"Item not found: {extracted_data['item_code']}")
            return {
                "messages": [AIMessage(content=_format_error_message({
        ...
        if result:
            ...
            success_message = _format_success_message(result, item_info, extracted_data["quantity"])
