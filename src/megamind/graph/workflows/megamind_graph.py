...
# Import nodes
from megamind.graph.nodes.orchestrator import (
    orchestrator_node,
    orchestrator_response_node,
)
...
def route_after_orchestrator(state: AgentState) -> str:
    ...
    - respond_streaming: Route to Phase 2 for streaming response
    - route: Route to a specialist
    - respond: End (after Phase 2 completes)
    """
    ...
    if next_action == "respond_streaming":
        # Route to Phase 2 for streaming response
        logger.debug("Routing to: orchestrator_response_node (Phase 2)")
        return "orchestrator_response_node"
...
async def build_megamind_graph(checkpointer: Optional[AsyncPostgresSaver] = None):
    ...
    workflow.add_node("orchestrator_response_node", orchestrator_response_node)
    ...
    # Orchestrator routing (Phase 1 -> Phase 2 or subagent)
    workflow.add_conditional_edges(
        "orchestrator_node",
        route_after_orchestrator,
    )

    # Phase 2 response node goes to END
    workflow.add_edge("orchestrator_response_node", END)
