...
def test_router_prompt_parsing():
    """Test that the router prompt includes the new stock movement agent."""
    # Testing router prompt...
    
    # Test queries that should route to stock movement agent
    ...
    # Test queries that should route to general agent
    general_queries = [
        "create a sales invoice",
        "update employee record",
        "give me company list",
        "create purchase order"
    ]
    
    # Test queries that should route to rag
    rag_queries = [
        "show me the latest sales invoice document",
        "retrieve employee contract",
        "what is in the documents"
    ]
    
    print("✓ Router prompt includes stock_movement_agent_node")
    print("✓ Test queries defined for all three agents")
    assert "stock_movement_agent_node" in router_node_instructions


def test_route_schema():
    """Test that the Route schema supports the new agent."""
    print("\nTesting Route schema...")
    
    try:
        # Test valid routes
        route1 = Route(next_node="rag_node")
        route2 = Route(next_node="agent_node") 
        route3 = Route(next_node="stock_movement_agent_node")
        
        print("✓ All three route options are valid")
        print(f"  - rag_node: {route1.next_node}")
        print(f"  - agent_node: {route2.next_node}")
        print(f"  - stock_movement_agent_node: {route3.next_node}")
        
        assert True
    except Exception as e:
        print(f"✗ Route schema test failed: {e}")
        assert False, f"Route schema test failed: {e}"
...
def test_prompts():
    ...
    try:
        ...
        print(f"  - stock_movement_agent_instructions: {len(stock_movement_agent_instructions)} chars")
        print(f"  - router_node_instructions: {len(router_node_instructions)} chars")
        ...
        key_terms = ["stock entry", "material request", "stock reconciliation", "inventory"]
